...
export async function getSearchedNews({
  ...
  try {
    ...
    if (!res.ok)
      throw new Error("something went wrong while fetching the news");
    const response = await res.json();
    return response;
  } catch {
...
export async function getNewsByCategory({ category }: { category: string }) {
  try {
    const res = await fetch(
      `https://newsapi.org/v2/top-headlines?country=us&category=${category}&pageSize=4&page=1&apiKey=${API_KEY}`
    );
    if (!res.ok)
      throw new Error(
        "error while fetching the news according to the category"
      );
    const response = await res.json();
    return response;
  } catch {
